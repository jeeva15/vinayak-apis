...
$campaignlist = $campaign->getVehicleList();
...
<center>
	...
	<div align="center">
		...
		<input type="hidden" name="hdnAction" id="hdnAction">
			<br>
		<a href="createvehicle.php"><input type="button" name="createuser" value="Create Vehicle" class="button"></a>
		<br><br>
		<table cellpadding="1" cellspacing="1" border="0" bgcolor="#EEEEEE" width="100%" class="mediumtxt">
			...
			<tr id="row_<?php echo $i?>" bgcolor="#FFFFFF">
				<td><?php echo $i?></td>				
				<td><?php echo $campaignval["vehicleNumber"];?></a></td>
				
				<td><?php if($campaignval["vehicleStatus"] == 1) { echo "Active";} else{ echo "In Active";}?></td>
				
				<td><a href="#" onclick='_getBox("editvehicle.php?page=Edit&ac=<?php echo $commonObj->Encrypt($campaignval["vehicleId"]);?>","40%","40%")'><img src="images/edit.gif" border="0" alt="edit" title="Edit"></a> &nbsp; &nbsp;<a href="javascript:void(0)" onclick="confimuser('<?php echo $commonObj->Encrypt($campaignval["vehicleId"]);?>','<?php echo $i?>');"><img src="images/close.gif" border="0" alt="Delete" title="Delete"></td>
	...
	<script type="text/javascript">
		...
		function DeleteUser(id){						
			str = id.split('~~~~~');
			$.ajax({
				type: "POST", url: 'vehicleaction.php', data: "id="+str[0]+"&hAct=3",
